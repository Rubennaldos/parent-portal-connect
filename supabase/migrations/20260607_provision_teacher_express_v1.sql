-- =============================================================================
-- Provision Express de Profesores — v1
-- Reemplaza insert_teacher_express (que intentaba INSERT manual en auth.users).
--
-- CAMBIO ARQUITECTÓNICO:
--   La creación del usuario en Auth la hace la Edge Function teacher-express
--   usando supabaseAdmin.auth.admin.createUser() (mismo patrón que express-enrollment).
--   Este RPC SOLO toca tablas públicas: profiles + teacher_profiles + audit_logs.
--   Sin contacto con auth.* → sin dependencia de instance_id.
-- =============================================================================

-- Limpiar versión rota
...
--             p_school_id, p_actor_id (quien llama, para auditoría).
-- =============================================================================
...
BEGIN
  ---------------------------------------------------------------------------
  -- 0) Validación de actor (fail-closed)
  ---------------------------------------------------------------------------
  ...
  IF p_school_id IS NULL THEN
    RAISE EXCEPTION
      'ERR_TEACHER_INVALID_INPUT: Debes indicar una sede válida.';
  END IF;
  ...
  ---------------------------------------------------------------------------
  -- 3) Unicidad de DNI en tablas públicas
  ---------------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------------
  -- 4) Transacción atómica: solo tablas públicas
  ...
  ---------------------------------------------------------------------------
  -- 5) Auditoría — columnas reales de audit_logs:
