-- ============================================================================
-- v8.3 — process_traditional_voucher_approval
-- Fecha: 2026-04-29
--
-- BUGS CERRADOS:
--
-- [BUG-1 CRÍTICO] Bloque already_complete llamaba fn_ensure SOLO si existía una
--   compra pending/partial con lunch_order_id del RR. Si el lunch_order no tenía
--   NINGUNA transacción (ni pending ni paid), fn_ensure nunca creaba el espejo
--   en reintento → padre veía deuda para siempre. CORRECCIÓN: fn_ensure se llama
--   SIEMPRE que haya lunch_order_ids activos (fn_ensure es idempotente).
--
-- [BUG-2 MODERADO] fn_ensure UPDATE masivo pending→paid no filtraba student_id.
--   En teoría podría marcar pagada una compra de otro alumno si compartieran
--   lunch_order_id (imposible hoy, pero sin constraint). CORRECCIÓN: ambas
--   definiciones (20260422 y 20260425) ahora incluyen:
--   AND (t.student_id = p_student_id OR t.student_id IS NULL).
--
-- [BUG-3 MODERADO] v8.1 eliminó la llamada a fn_auto_apply_balance_to_pending_purchases
--   al final del flujo normal (que había añadido 20260423_fix_auto_apply).
--   CORRECCIÓN: restaurada aquí.
--
-- Base: v8.1 + corrección bloque already_complete + student_id guard + auto-apply.
-- ============================================================================

-- Índice unique parcial (idempotente — no falla si ya existe)
...
-- ─────────────────────────────────────────────────────────────────────────────
-- process_traditional_voucher_approval v8.3
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  IF v_req.status = 'approved' THEN
    IF EXISTS (
      ...
      -- v8.3: alinear espejos de almuerzo SIEMPRE que haya lunch_order_ids
      -- (fn_ensure es idempotente: solo inserta si aún no existe espejo paid).
      -- No limitamos a "si hay pending": fn_ensure también crea espejo cuando
      -- no existe NINGUNA transacción, que es el caso más crítico de reintento.
      IF v_req.lunch_order_ids IS NOT NULL
         AND array_length(v_req.lunch_order_ids, 1) > 0
         AND EXISTS (
           SELECT 1 FROM public.lunch_orders lo
           WHERE lo.id = ANY(v_req.lunch_order_ids)
  ...
  IF v_req.request_type = 'lunch_payment'
     AND COALESCE(cardinality(v_lunch_ids), 0) > 0
  THEN
    -- Columna correcta: final_price (NO lo.price)
  ...
  -- ── v8.2+: auto-aplicar saldo a compras pendientes (restaurado tras v8.1) ──
  v_auto_applied_ids := public.fn_auto_apply_balance_to_pending_purchases(
    v_req.student_id,
    v_req.school_id,
    p_admin_id,
  ...
  -- ── billing_queue: ON CONFLICT idempotente ────────────────────────────────
  -- FIX v8.1: único cambio respecto al v7.4 original.
  -- Si se reintenta la aprobación, el row existente se resetea a 'pending'
  -- en lugar de fallar con duplicate key.
  ...
  THEN
    ...
    -- RETURNING no siempre devuelve id en DO UPDATE; leer si es null
...
COMMENT ON FUNCTION public.process_traditional_voucher_approval(uuid, uuid) IS
  'v8.3 2026-04-29 — v8.1 + fn_auto_apply_balance_to_pending_purchases al final '
  '(restaurado; v8.1 lo había eliminado) + antes del RETURN already_complete: '
  'fn_ensure si quedan lunch purchases pending/partial + auto-apply de saldo.';

SELECT 'v8.3 OK: process_traditional_voucher_approval — lunch parity early exit + auto-apply' AS resultado;
