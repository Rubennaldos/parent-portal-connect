-- ============================================================
-- REGLA ÚNICA DE STOCK EN POS — guiado por fila en product_stock
-- ============================================================
-- Problema previo: el bloqueo de venta dependía de
--   products.stock_control_enabled = true POR PRODUCTO.
--
-- Nueva regla (alineada con el switch global):
--   Si existe una fila activa (is_enabled=true) en product_stock
--   para ese producto/sede → el stock controla la venta.
--   Si NO existe fila → el producto es venta libre (no bloquea).
--
-- Cambios quirúrgicos en complete_pos_sale_v2:

--   2. Validación combo  → usa existencia de fila en lugar del flag
--   3. Validación línea  → ídem
--   4. Descuento paso 13 → ídem
--
-- NO toca: RLS, roles, saldos, idempotencia, pasarela, triggers.
-- ============================================================
...
DECLARE
  ...
  -- CAMBIO: v_has_stock_ctrl ahora significa "hay fila activa en product_stock"
...
BEGIN
  ...
  -- CAMBIO: eliminado filtro p.stock_control_enabled = true.
  -- Ahora bloquea toda fila activa de los productos en la venta.
  ...
  IF NOT v_allow_negative_stock THEN
    ...
    LOOP
      ...
      -- CAMBIO: v_has_stock_ctrl = existe fila activa (no depende de stock_control_enabled)
  ...
  LOOP
    ...
    ELSE
      ...
      -- CAMBIO: v_has_stock_ctrl = existe fila activa en product_stock (no stock_control_enabled)
      SELECT ps.current_stock IS NOT NULL,
             COALESCE(ps.current_stock, 0)
      INTO   v_has_stock_ctrl, v_current_stock
      FROM   products p
  ...
  LOOP
    ...
    -- CAMBIO: descuenta si hay fila activa en product_stock (no depende de stock_control_enabled).
    ...
    -- Guardia explícita: interceptar antes de que el trigger de BD dispare STOCK_BLOQUEADO.
    -- Si no hay stock suficiente, lanzamos INSUFFICIENT_STOCK con el mensaje amigable.
    ...
    -- UPDATE directo (la fila ya existe — verificado arriba con FOR UPDATE).
    -- Se evita el INSERT ... ON CONFLICT para no disparar el trigger BEFORE INSERT
    -- con un valor inicial negativo.
...
SELECT 'OK: complete_pos_sale_v2 — bloqueo por fila de stock, no por flag' AS resultado;
