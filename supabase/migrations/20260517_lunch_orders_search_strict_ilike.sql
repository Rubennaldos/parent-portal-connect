-- ============================================================
-- Lunch orders search: strict name matching with ILIKE
-- ============================================================
-- Objetivo:
-- 1) Reemplazar similitud trigram (%) por contencion estricta.
-- 2) Mantener candados por sede, estado y tipo de persona.
-- 3) No forzar is_cancelled en el RPC; lo controla el filtro UI.
...
BEGIN
  ...
  RETURN QUERY
    ...
    WHERE
      ...
      AND (
        public.normalize_search(COALESCE(lo.manual_name, '')) ILIKE '%' || v_norm_search || '%'
        OR public.normalize_search(COALESCE(st.full_name,  '')) ILIKE '%' || v_norm_search || '%'
        OR public.normalize_search(COALESCE(tp.full_name,  '')) ILIKE '%' || v_norm_search || '%'
      )
