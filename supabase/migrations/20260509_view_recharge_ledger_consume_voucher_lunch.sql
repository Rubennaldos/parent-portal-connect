-- ============================================================================
-- REC FIFO: contar almuerzos pagados con voucher de deuda/almuerzo
-- 2026-05-09
--
-- Problema: al aprobar voucher, process_traditional deja payment_method = lo
-- que eligió el padre (p. ej. transferencia), pero payment_source =
-- debt_voucher_payment / lunch_voucher_payment. La vista solo contaba consumo
-- REC si payment_method era saldo|balance|wallet_balance|mixto → el modal
-- mostraba REC intacto aunque la deuda de almuerzo ya estaba paid.
--
-- Regla nueva: mismo bloque de almuerzos paid + amount < 0, y además
--   (método interno) OR (origen voucher de deuda/almuerzo en metadata).
-- No amplía tarjeta/efectivo sin payment_source (evita ruido POS).
-- ============================================================================
...
student_consumed AS (
  ...
  FROM (
    ...
    -- Almuerzo: saldo interno O pago aprobado por voucher (metadata.payment_source)
    SELECT
      t.student_id,
      COALESCE(SUM(ABS(t.amount)), 0)::numeric AS consumed_amount
    FROM public.transactions t
    WHERE t.student_id                     IS NOT NULL
      ...
      AND t.amount                         < 0
      AND (
        lower(trim(COALESCE(t.payment_method, ''))) IN (
          'saldo', 'balance', 'wallet_balance', 'mixto'
        )
        OR COALESCE(t.metadata->>'payment_source', '') IN (
          'debt_voucher_payment', 'lunch_voucher_payment'
        )
      )
    GROUP BY t.student_id
  ) x
...
COMMENT ON VIEW public.view_recharge_ledger IS
  ...
  '+ almuerzos paid con saldo/balance/wallet_balance/mixto '
  '+ almuerzos paid vía voucher de deuda/almuerzo (metadata.payment_source).';
