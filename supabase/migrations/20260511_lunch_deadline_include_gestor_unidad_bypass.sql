-- Incluye gestor_unidad en el bypass de ORDER_DEADLINE para pedidos de almuerzo.
-- No elimina reglas: solo alinea "admin de sede" (gestor_unidad) con admin_general
-- respecto al límite horario global.
...
BEGIN
  ...
  IF EXISTS (
    ...
    WHERE  p.id = auth.uid()
      AND  p.role IN ('admin', 'admin_general', 'superadmin', 'admin_sede', 'gestor_unidad')
  ) THEN
    RETURN json_build_object(
      'can_order', true,
...
BEGIN
  ...
  IF v_caller_role IN ('admin', 'admin_general', 'superadmin', 'admin_sede', 'gestor_unidad') THEN
    RETURN NEW;
  END IF;
...
COMMENT ON FUNCTION public.check_order_eligibility(DATE, UUID) IS
  'Devuelve {can_order, reason}. Reloj Lima + deadline global. Roles admin/admin_general/superadmin/admin_sede/gestor_unidad omiten solo el tope horario; orders_enabled por sede sigue aplicando.';

COMMENT ON FUNCTION public.tg_validate_lunch_order_deadline() IS
  'BEFORE INSERT lunch_orders. Rechaza si plazo global venció. Bypass: admin, admin_general, superadmin, admin_sede, gestor_unidad.';

SELECT '20260511_lunch_deadline_include_gestor_unidad_bypass ✅ listo' AS resultado;
