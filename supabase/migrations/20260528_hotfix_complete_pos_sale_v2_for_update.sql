-- ============================================================
-- HOTFIX: complete_pos_sale_v2 — FOR UPDATE en outer join
-- ============================================================
-- Problema en producción:
--   La función tiene una versión antigua donde un LEFT JOIN y un
--   FOR UPDATE se combinaban en la misma query (paso 4 o paso 13),
--   causando: "FOR UPDATE cannot be applied to the nullable side
--   of an outer join" (PostgreSQL error 0A000).
--
-- Solución:
--   Redeploy de la versión limpia (alineada con 20260524).
--   Reglas preservadas intactas:
--     - FOR UPDATE en students (paso 2): tabla directa, sin join
--     - FOR UPDATE OF ps (paso 3): tabla directa product_stock, sin LEFT JOIN
--     - LEFT JOIN en paso 4: solo lectura, SIN FOR UPDATE
--     - LEFT JOIN en paso 3b: solo lectura, SIN FOR UPDATE
--     - FOR UPDATE en paso 13: tabla directa product_stock, sin join
--   Nada cambia en: RLS, saldos, topes, idempotencia, kardex.
-- ============================================================
...
DECLARE
  ...
  -- v_has_stock_ctrl = existe fila activa en product_stock para ese producto/sede
...
BEGIN
  ...
  -- Esto es obligatorio para evitar el error "FOR UPDATE cannot be
  ...
  -- REGLA: FOR UPDATE OF ps sobre tabla DIRECTA (product_stock ps),
  -- SIN LEFT JOIN. El filtro usa un INNER JOIN implícito (subquery de IDs).
  ...
  -- REGLA: LEFT JOIN aquí es solo lectura, SIN FOR UPDATE.
  ...
  LOOP
    ...
    ELSE
      ...
      -- LEFT JOIN de solo lectura — NO tiene FOR UPDATE (eso causaría el error)
      SELECT ps.current_stock IS NOT NULL,
             COALESCE(ps.current_stock, 0)
      INTO   v_has_stock_ctrl, v_current_stock
      FROM   products p
  ...
  LOOP
    ...
    -- Verifica existencia de fila activa antes del lock (evita error si no existe)
    ...
    -- REGLA: FOR UPDATE sobre tabla directa product_stock, SIN LEFT JOIN.
...
SELECT 'HOTFIX OK: complete_pos_sale_v2 — sin FOR UPDATE en outer join' AS resultado;
