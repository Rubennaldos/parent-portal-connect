-- ============================================================================
-- Bypass de plazo global de almuerzo: operador_caja + cajero
-- Fecha: 2026-05-14
--
-- Contexto:
--   tg_validate_lunch_order_deadline / check_order_eligibility ya omitían el
--   tope horario global para admin/admin_general/superadmin/admin_sede/gestor_unidad.
--   operador_caja y cajero quedaban sujetos al mismo corte que un padre, por eso
--   el INSERT a lunch_orders fallaba con ORDER_DEADLINE aun teniendo permiso UI.
--
-- Cambio:
--   Solo SE AÑADEN roles al bypass existente. No se eliminan reglas ni roles.
--   orders_enabled por sede y el resto de la lógica de check_order_eligibility
--   se mantienen.
--
-- Riesgo de negocio (transparente):
--   Personal de caja podrá registrar pedidos después del corte global para la
--   fecha objetivo, igual que la sede/gestor ya podían con bypass administrativo.
-- ============================================================================
...
BEGIN
  ...
  IF EXISTS (
    ...
    RETURN json_build_object(
      'can_order', true,
      'reason',    'Pedido permitido (rol staff; sin límite de hora global).'
    );
  END IF;
...
COMMENT ON FUNCTION public.check_order_eligibility(DATE, UUID) IS
  'Devuelve {can_order, reason}. Reloj Lima + deadline global. Roles admin/admin_general/superadmin/admin_sede/gestor_unidad/operador_caja/cajero omiten solo el tope horario; orders_enabled por sede sigue aplicando.';

COMMENT ON FUNCTION public.tg_validate_lunch_order_deadline() IS
  'BEFORE INSERT lunch_orders. Rechaza si plazo global venció. Bypass: admin, admin_general, superadmin, admin_sede, gestor_unidad, operador_caja, cajero.';

SELECT '20260514_lunch_deadline_bypass_cajero_operador_caja aplicado' AS resultado;
