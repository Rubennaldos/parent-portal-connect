-- ============================================================
-- FIX CRÍTICO: CXC muestra solo 50 deudores en lugar de todos
-- ============================================================
-- Síntoma: el modal "CXC — Cuentas por Cobrar" mostraba un tope
--   de 50 deudores aunque hubiera 126+ en la sede.
--
-- Causa raíz: el DEFAULT del parámetro p_limit era 50. Cuando el
--   frontend (buildCxcListAsync) enviaba p_limit=9999, el RPC
--   funcionaba bien. Pero si la función desplegada en Supabase era
--   una versión ANTERIOR sin ese parámetro, PostgREST lo ignoraba
--   y ejecutaba con LIMIT 50 implícito.
--
-- Corrección dual:
--   1. Elevar el DEFAULT de p_limit a 2000 (cubre cualquier colegio
--      de tamaño razonable sin necesitar el parámetro explícito).
--   2. Usar LEAST(GREATEST(p_limit, 1), 5000) para que p_limit=9999
--      del frontend quede fijo en 5000 (suficiente, y evita timeouts
--      en colegios con miles de deudores en el futuro).
--   3. DROP previo para asegurarse de que cualquier versión antigua
--      con firma idéntica sea reemplazada.
-- ============================================================
...
CREATE OR REPLACE FUNCTION get_billing_consolidated_debtors(
  ...
  p_limit            integer     DEFAULT 2000   -- ← SUBIDO de 50 a 2000
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  ELSE
    -- admin de sede solo ve su propia sede
    v_eff_school_id := v_caller_school;
  END IF;

  -- ── CAP de seguridad: entre 1 y 5000 ─────────────────────────
  -- Evita timeouts si alguien mandara un p_limit absurdo (ej. 999999)
  -- y garantiza que p_limit=9999 del CXC frontend devuelva todos los
  -- deudores reales (ningún colegio tiene >5000 deudores activos).
  ...
  FROM (
    ...
    LIMIT  v_safe_limit    -- ← usa la variable con el cap aplicado
    OFFSET p_offset
  ) sub;
