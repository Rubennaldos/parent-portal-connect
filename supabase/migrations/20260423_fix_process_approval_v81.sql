-- ============================================================================
-- CORRECCIÓN CRÍTICA v8.1 — process_traditional_voucher_approval
-- Fecha: 2026-04-23
--
-- CAUSA DEL ERROR "column lo.price does not exist":
--   La migración 20260423_emergency_kiosk_billing_fix.sql (v8.0) reescribió
--   toda la función con un FIFO manual que usaba lo.price en vez de lo.final_price.
--   Además descartó fn_ensure_paid_purchase_mirrors_for_lunch_voucher_approval
--   que es la función correcta para espejo de almuerzos.
--
-- ESTA MIGRACIÓN:
--   1. Restaura el cuerpo EXACTO de la v7.4 (de 20260422_lunch_debt_parity_vouchers_rls.sql)
--   2. Aplica SOLO el ON CONFLICT en billing_queue INSERT (el único fix necesario)
--   3. No toca ninguna otra lógica — cero riesgo de regresión
--
-- GARANTÍA:
--   ON CONFLICT (recharge_request_id) WHERE recharge_request_id IS NOT NULL
--   requiere el índice uq_billing_queue_recharge_request creado en
--   20260423_emergency_kiosk_billing_fix.sql. Si aún no existe, se recrea aquí.
-- ============================================================================
...
-- ─────────────────────────────────────────────────────────────────────────────
-- process_traditional_voucher_approval v8.1
-- Cuerpo idéntico a v7.4 + ON CONFLICT en billing_queue
-- ─────────────────────────────────────────────────────────────────────────────
...
COMMENT ON FUNCTION public.process_traditional_voucher_approval(uuid, uuid) IS
  'v8.1 2026-04-23 — Restaura el cuerpo EXACTO de la v7.4 (columnas correctas: lo.final_price, '
  'fn_ensure_paid_purchase_mirrors_for_lunch_voucher_approval). '
  'Único cambio vs v7.4: billing_queue INSERT ahora usa ON CONFLICT DO UPDATE '
  '(idempotente en reintento). '
  'v8.0 eliminada: introdujo lo.price (inexistente) causando error en aprobaciones.';

SELECT 'v8.1 OK: process_traditional_voucher_approval restaurada con lo.final_price y ON CONFLICT' AS resultado;
