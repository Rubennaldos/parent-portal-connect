-- ============================================================
-- FIX: get_billing_consolidated_debtors — UNION uuid/text
-- ============================================================
-- Síntoma:  "UNION types uuid and text cannot be matched"
-- Causa típica: una rama del UNION ALL devuelve tipos distintos
--   (ej. payment_status como ENUM vs text, varchar vs text,
--   o columnas uuid mezcladas con texto en student_id/teacher_id).
-- Solución: casts explícitos idénticos en real_pending y virtual_lunch.
--
-- Además: alias "p." en la consulta a profiles (consistencia con otros RPC).
-- ============================================================
...
BEGIN
  ...
  -- ── Lógica de deudores (UNION con tipos alineados) ────────
  WITH
