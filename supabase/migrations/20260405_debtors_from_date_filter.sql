-- ============================================================
-- FILTRO "COBRAR DESDE" — get_billing_consolidated_debtors
-- ============================================================
-- La dueña quiere cobrar por rango de fechas, no solo "hasta".
-- Se agrega p_from_date (fecha mínima de creación de la deuda).
--
-- Cambios:
--   1. Nueva firma: (..., p_from_date timestamptz DEFAULT NULL, ...)
--   2. En real_pending: AND (p_from_date IS NULL OR t.created_at >= p_from_date)
--   3. En virtual_lunch: AND (p_from_date IS NULL OR lo.order_date >= p_from_date::date)
-- ============================================================
...
CREATE OR REPLACE FUNCTION get_billing_consolidated_debtors(
  ...
  p_from_date        timestamptz DEFAULT NULL   -- ← NUEVO: filtro "cobrar desde"
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  real_pending AS (
    ...
    WHERE t.type           = 'purchase'
      ...
      AND (p_from_date  IS NULL OR t.created_at >= p_from_date)   -- ← NUEVO
  ...
  virtual_lunch AS (
    ...
    WHERE lo.is_cancelled   = false
      ...
      AND (p_from_date  IS NULL OR lo.order_date >= p_from_date::date)  -- ← NUEVO
