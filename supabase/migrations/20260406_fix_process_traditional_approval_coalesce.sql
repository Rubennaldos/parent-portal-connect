-- ═══════════════════════════════════════════════════════════════════════════
-- FIX: process_traditional_voucher_approval
-- Error: COALESCE types uuid[] and jsonb cannot be matched (42804)
--
-- CAUSA: Línea del RETURN jsonb_build_object tenía:
--   COALESCE(v_updated_ids, '[]'::jsonb)
-- Pero v_updated_ids es uuid[], y '[]'::jsonb es jsonb — tipos incompatibles.
--
-- SOLUCIÓN: Cambiar a COALESCE(v_updated_ids, '{}'::uuid[])
-- PostgreSQL convierte automáticamente uuid[] a JSON array al incluirlo
-- dentro de jsonb_build_object. El resultado en el cliente es idéntico.
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'updated_tx_ids',      COALESCE(v_updated_ids, '{}'::uuid[]),   -- ✅ FIX: uuid[] en vez de jsonb
    'amount',              v_req.amount,
    'student_id',          v_req.student_id,
...
SELECT 'FIX APLICADO: COALESCE(v_updated_ids, uuid[]) corregido' AS resultado;
