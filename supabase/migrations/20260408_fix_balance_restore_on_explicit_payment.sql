-- ═══════════════════════════════════════════════════════════════════════════
-- MIGRATION: process_traditional_voucher_approval v3
-- Fecha: 2026-04-08
--
-- PROBLEMA RESUELTO (el más importante hasta ahora):
--   Cuando el padre seleccionaba tickets explícitamente (paid_transaction_ids
--   tenía valores), el RPC marcaba los tickets como 'paid' correctamente,
--   pero NUNCA llamaba a adjust_student_balance para restaurar el balance.
--
--   Resultado: 115 alumnos en producción con balance negativo residual,
--   a quienes el sistema les seguía cobrando deudas ya pagadas.
--
-- CAUSA:
--   El PASO 6b (ajuste de balance) solo se ejecutaba cuando v_needs_fifo = true
--   (es decir, cuando NO había IDs explícitos). Para pagos con IDs explícitos,
--   v_needs_fifo = false, y el balance jamás se restauraba.
--
-- SOLUCIÓN:
--   Se agrega PASO 6c que corre cuando v_needs_fifo = false pero hay
--   tickets de kiosco (sin lunch_order_id) que fueron actualizados.
--   Calcula la suma de esos tickets y restaura el balance hasta 0 como máximo.
--   Nunca crea crédito positivo — solo limpia la deuda residual.
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  IF NOT v_is_partial THEN
    ...
    -- Recopilar IDs efectivamente actualizados en esta aprobación
    ...
    -- ── PASO 6b: AJUSTE DE BALANCE — CAMINO FIFO (sin IDs explícitos) ───────
    -- Cuando el padre pagó "Saldo negativo kiosco" (ID sintético),
    -- el monto sobrante después del FIFO se usa para recuperar el balance.
    ...
    -- ── PASO 6c: AJUSTE DE BALANCE — CAMINO EXPLÍCITO (IDs seleccionados) ───
    -- Cuando el padre seleccionó tickets específicos de kiosco, el RPC los
    -- marca como 'paid' PERO el balance nunca se restauraba. Este paso lo corrige.
    -- Solo aplica para tickets de kiosco (sin lunch_order_id en metadata).
    -- Nunca crea saldo positivo: el tope máximo es llevar el balance a 0.
...
COMMENT ON FUNCTION process_traditional_voucher_approval IS
  'v3 (2026-04-08): Agrega PASO 6c — restaura el balance del alumno cuando se pagan '
  'tickets de kiosco con IDs explícitos (paid_transaction_ids). '
  'Corrige el bug raíz que dejó 115 alumnos con balance negativo residual: '
  'el RPC marcaba tickets como paid pero nunca llamaba adjust_student_balance. '
  'El tope es siempre 0 — nunca crea crédito positivo con este mecanismo.';
