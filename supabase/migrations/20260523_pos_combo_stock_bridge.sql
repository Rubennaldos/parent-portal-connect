-- ============================================================
-- POS COMBO STOCK BRIDGE
-- ============================================================
-- Conecta la venta de combos con el inventario de sus componentes.
-- Cambios quirúrgicos — NO modifica RLS, roles, ni el flujo de
-- productos individuales ni ventas custom puras.
--
-- Cambio 1: get_active_combos_for_school
--   → agrega is_available_by_stock (flag listo para el frontend)
--   → agrega current_stock por componente en el JSON products
--
-- Cambio 2: complete_pos_sale_v2
--   → construye v_stock_lines expandiendo combos desde combo_items
--   → valida stock de componentes (paso 3b)
--   → descuenta y registra kardex por componente (paso 13)
--   → ventas libres puras (is_custom sin combo_id) no se tocan
-- ============================================================


-- ── 1. get_active_combos_for_school ─────────────────────────────────────────
...
BEGIN
  ...
  v_allow_neg := COALESCE(get_allow_negative_stock(), false);
  ...
  SELECT
    ...
    json_agg(
      json_build_object(
        ...
        'has_stock',     p.stock_control_enabled,
        'price',         p.price_sale,
        'current_stock', COALESCE(ps.current_stock, 0)
      )
      ORDER BY p.name
    )::json AS products,
    CASE
      WHEN v_allow_neg THEN true
      ELSE bool_and(
        NOT COALESCE(p.stock_control_enabled, false)
        OR COALESCE(ps.current_stock, 0) >= ci.quantity
      )
    END AS is_available_by_stock
...
SELECT 'OK: get_active_combos_for_school con is_available_by_stock' AS resultado;


-- ── 2. complete_pos_sale_v2 con expansión de stock para combos ───────────────
...
DECLARE
  ...
  -- Variables para expansión de combos
  v_combo_id    uuid;
  v_combo_name  text;
...
BEGIN
  ...
  -- Recorre p_lines y construye v_stock_lines con los productos que
  -- necesitan movimiento de inventario:
  --   · Producto normal (is_custom=false, product_id presente): 1-a-1.
  --   · Combo (is_custom=true + combo_id presente): expande desde combo_items.
  --   · Venta libre pura (is_custom=true sin combo_id): no toca stock.
  FOR v_elem IN
  ...
  LOOP
    ...
    ELSIF v_is_custom AND v_combo_id IS NOT NULL THEN
      -- Combo: expandir componentes multiplicando por cantidad del combo en carrito
      FOR v_ci_rec IN
        SELECT
    ...
    END IF;
    -- Venta libre pura (is_custom=true, sin combo_id): no agrega nada a v_stock_lines
  END LOOP;

  -- ── 3. BLOQUEAR filas de product_stock (orden determinista) ─────
  -- Fuente: v_stock_lines (productos normales + componentes de combos).
  ...
  -- Los productos normales se validan en paso 4.
  -- Los componentes de combo se validan aquí para poder lanzar
  -- INSUFFICIENT_STOCK antes de tocar transacciones o saldo.
  ...
  -- Sin cambios respecto a la versión anterior. Los combos entran aquí
  -- como is_custom=true con custom_price, igual que antes.
  FOR v_elem IN
    SELECT value FROM jsonb_array_elements(p_lines) AS value
  LOOP
    v_product_id   := (v_elem->>'product_id')::uuid;
  ...
  -- Fuente: v_stock_lines, que contiene:
  --   · Productos normales (is_combo=false)
  --   · Componentes de combos expandidos (is_combo=true)
  -- Ventas libres puras no están en v_stock_lines → sin impacto en stock.
...
SELECT 'OK: complete_pos_sale_v2 con expansión de stock para combos' AS resultado;
