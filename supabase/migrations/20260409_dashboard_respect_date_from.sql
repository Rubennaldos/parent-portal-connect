-- ============================================================
-- FIX: get_billing_dashboard_stats — respetar p_date_from
-- ============================================================
-- PROBLEMA: la versión anterior ignoraba p_date_from para
-- deuda pendiente (mostraba toda la historia), lo que causaba
-- que Dashboard y CXC mostraran números distintos cuando la
-- dueña elegía un rango de fechas.
--
-- FIX: cuando p_date_from está presente, filtrar también por
-- límite inferior en deuda pendiente y almuerzos virtuales.
-- Así Dashboard y CXC muestran exactamente el mismo número
-- para el mismo rango seleccionado.
-- ============================================================
...
DECLARE
  ...
  -- ── ALMUERZOS VIRTUALES (lunch_orders sin tx) ──
...
BEGIN
  ...
  --    Se respeta el rango completo: p_date_from y p_date_to
  --    Así coincide con lo que muestra el CXC para el mismo rango
  -- ============================================================
  SELECT
  ...
  WHERE t.type = 'purchase'
    ...
    AND t.created_at >= v_period_start   -- respetar fecha inicio
    AND t.created_at <= v_period_end     -- respetar fecha fin
    AND (p_school_id IS NULL OR t.school_id = p_school_id);

  -- ============================================================
  -- 2. ALMUERZOS VIRTUALES — lunch_orders sin transacción real
  --    Se respeta el rango completo de fechas
  -- ============================================================
  SELECT
    COALESCE(SUM(
      CASE
  ...
  WHERE lo.is_cancelled = false
    ...
    AND lo.order_date >= p_date_from     -- respetar fecha inicio
    AND lo.order_date <= p_date_to       -- respetar fecha fin
    AND (
      p_school_id IS NULL
  ...
  --    Aquí siempre aplica el rango completo
  -- ============================================================
  SELECT
  ...
  -- ============================================================
  -- 4. ANTIGÜEDAD DE LA DEUDA — respeta el rango seleccionado
  -- ============================================================
  SELECT
  ...
  -- ============================================================
  -- 5. TOP 15 DEUDORES — respeta el rango seleccionado
  -- ============================================================
  SELECT jsonb_agg(
    jsonb_build_object(
  ...
  -- ============================================================
  -- 6. RESUMEN POR SEDE — respeta el rango seleccionado
  -- ============================================================
  SELECT jsonb_agg(
    jsonb_build_object(
      'school_name',      COALESCE(s.name, 'Sin sede'),
  ...
  FROM (
    SELECT
      school_id,
      SUM(ABS(amount)) FILTER (WHERE payment_status IN ('pending','partial') AND created_at BETWEEN v_period_start AND v_period_end) AS pending,
      SUM(ABS(amount)) FILTER (WHERE payment_status IN ('pending','partial') AND (metadata->>'lunch_order_id') IS NOT NULL AND created_at BETWEEN v_period_start AND v_period_end) AS lunch_p,
      SUM(ABS(amount)) FILTER (WHERE payment_status IN ('pending','partial') AND (metadata->>'lunch_order_id') IS NULL AND created_at BETWEEN v_period_start AND v_period_end) AS cafe_p,
      ...
      COUNT(DISTINCT COALESCE(student_id::text, teacher_id::text, lower(trim(manual_client_name)))) FILTER (WHERE payment_status IN ('pending','partial') AND created_at BETWEEN v_period_start AND v_period_end) AS debtors_count
    FROM transactions
    WHERE type = 'purchase'
      AND is_deleted = false
      AND (p_school_id IS NULL OR school_id = p_school_id)
