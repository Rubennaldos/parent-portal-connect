-- ============================================================
-- MIGRACIÓN CRÍTICA: Crear view_student_debts en producción
-- + Fix: payment_method IS NULL también cuenta como deuda
--
-- Problema detectado:
--   1. La vista view_student_debts nunca fue creada en producción.
--      Ambas RPCs (get_parent_debts, get_billing_consolidated_debtors)
--      dependían de ella pero usaban código legacy sin la vista.
--   2. Los lunch_orders con payment_method = NULL no aparecían
--      como deuda porque el filtro solo aceptaba 'pagar_luego'.
--      Esto causaba que padres que pidieron almuerzos sin
--      especificar payment_method vieran "Sin deudas pendientes".
--
-- Fix:
--   - payment_method = 'pagar_luego' OR payment_method IS NULL
--     → ambos casos se tratan igual: pagar después
-- ============================================================


-- ── PASO 1: Crear view_student_debts ────────────────────────────────
...
-- TRAMO 2 — Almuerzos sin transacción registrada aún
-- FIX: payment_method IS NULL también cuenta como "pagar después"
SELECT
...
-- TRAMO 3 — Saldo negativo del kiosco (solo para portal de padres)
SELECT
...
-- ── PASO 2: Permisos sobre la vista ─────────────────────────────────

GRANT SELECT ON view_student_debts TO authenticated, service_role;


-- ── PASO 3: get_parent_debts — usa la vista ──────────────────────────
...
-- ── PASO 4: get_billing_consolidated_debtors — usa la vista ─────────
...
-- ── VERIFICACIÓN ─────────────────────────────────────────────────────
-- Corre esto después para confirmar que funciona:
-- SELECT fuente, COUNT(*), SUM(monto) FROM view_student_debts GROUP BY fuente;
-- SELECT * FROM get_parent_debts('b91d0674-0727-4e5e-8309-e00bd2ba6e15');
