...
function assertStringField(value: unknown, field: string): string {
  if (typeof value !== "string" || value.trim() === "") {
    throw new ExpressRpcError(
      ...
      `Respuesta RPC inv├ílida: falta campo obligatorio "${field}".`,
      { field, value },
    );
  }
  return value;
...
function toEnrollRpcRow(row: unknown): EnrollRpcRow {
  if (!row || typeof row !== "object") {
    throw new ExpressRpcError(
      ERR_EXPRESS_DATABASE_ROLLBACK,
      "Respuesta RPC inv├ílida: estructura vac├¡a o no-objeto.",
    );
  }
...
export async function enrollStudentViaRpc(input: EnrollRpcInput): Promise<EnrollRpcRow> {
  ...
  if (error) {
    ...
    if (looksLikeHierarchyError(message)) {
      throw new ExpressRpcError(
        ...
        "La jerarqu├¡a school/level/classroom no es v├ílida para la matr├¡cula express.",
        {
    ...
    throw new ExpressRpcError(
      ERR_EXPRESS_DATABASE_ROLLBACK,
      "Fallo de base de datos en matr├¡cula express. Operaci├│n revertida.",
      {
        db_message: error.message,
        db_code: error.code,
        db_hint: error.hint,
  ...
  try {
    ...
    throw new ExpressRpcError(
      ERR_EXPRESS_DATABASE_ROLLBACK,
      "Fallo validando la respuesta del RPC de matr├¡cula express.",
