...
function validateBody(body: any): { ok: true; value: ExpressRequest } | { ok: false; response: Response } {
  if (!body || typeof body !== "object") {
    return { ok: false, response: jsonErr(ERR_EXPRESS_DATABASE_ROLLBACK, "Payload inválido.", 400) };
  }
...
serve(async (req: Request) => {
  ...
  if (req.method !== "POST") {
    return jsonErr(ERR_EXPRESS_DATABASE_ROLLBACK, "Método no permitido.", 405);
  }
  ...
  try {
    ...
    if (!authHeader || !authHeader.toLowerCase().startsWith("bearer ")) {
      return jsonErr(ERR_EXPRESS_UNAUTHORIZED, "Token faltante o inválido.", 401);
    }
    ...
    // "Auth session missing!" porque no hay sesión local. La única forma de
    ...
    // 3) Validación directa: rawToken como argumento bypasea el check de sesión local
    const { data: authData, error: authError } = await supabaseAuthClient.auth.getUser(rawToken);
    if (authError || !authData?.user?.id) {
      return jsonErr(
        ERR_EXPRESS_UNAUTHORIZED,
        `Fallo al validar sesión: ${authError?.message || "Token inválido"}`,
        401,
      );
    }
    ...
    if (actorProfileError || !actorProfile?.role || !ALLOWED_ADMIN_ROLES.has(actorProfile.role)) {
      return jsonErr(
        ERR_EXPRESS_UNAUTHORIZED,
        "No tienes permisos para matrícula express.",
        403,
      );
    }
    ...
    return jsonErr(
      ERR_EXPRESS_DATABASE_ROLLBACK,
      "Fallo interno en matrícula express. Operación revertida.",
      500,
      { detail: message },
    );
  }
